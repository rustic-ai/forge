...
5. **Env build.** Environment variables are assembled from the (re-hydrated) guild spec, messaging config, and org context. The agent's `forge_extra_deps` is also restored from that re-hydrated spec when the incoming payload carries none — rustic-ai core does not model the field, so it is dropped whenever the Python `GuildManagerAgent` re-serializes a spec to issue the spawn.
